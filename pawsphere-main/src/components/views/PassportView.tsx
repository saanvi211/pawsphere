...
export const PassportView: React.FC<PetDigitalPassportProps> = ({ animal, onUpdateAnimal }) => {
  if (!animal) {
    return (
      <div className="p-12 text-center space-y-4 glass-panel-dark rounded-3xl border border-cyan-500/30 max-w-lg mx-auto my-12">
        ...
        <h3 className="text-2xl font-extrabold text-white glow-text-cyan">NO DIGITAL PASSPORT FOUND</h3>
        <p className="text-xs text-slate-300">
          Please register or select a pet companion to view their Official Digital Passport.
        </p>
      </div>
    );
  }
  ...
  const vaccinationsList = animal.vaccinations || [];
  ...
  const qrData = JSON.stringify({
    ...
    vaccinesCount: vaccinationsList.length
  });
    ...
    <div className="py-6 max-w-4xl mx-auto px-4 space-y-6">
      ...
      <div className="bg-white rounded-3xl border-4 border-brand-solidBlue p-6 sm:p-8 shadow-xl space-y-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          <PetImage pet={animal} className="w-full h-48 rounded-2xl object-cover border-4 border-brand-solidOrange shadow-sm" />
        ...
        <div className="border-t border-slate-200 pt-6 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-extrabold text-sm text-slate-900">Verified Vaccine Stamps ({vaccinationsList.length})</h4>
            <button
